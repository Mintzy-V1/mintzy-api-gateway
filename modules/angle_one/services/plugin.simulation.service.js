...
// Hardcoded auto-stop — 1:00 PM IST
const SIMULATION_AUTO_STOP_HOUR_IST = 13;
const SIMULATION_AUTO_STOP_MINUTE_IST = 0;
...
const isSimulationAutoStopDue = (date = new Date()) => {
    const { hour, minute } = getIstDateParts(date);
    if (hour !== SIMULATION_AUTO_STOP_HOUR_IST) return false;
    return minute >= SIMULATION_AUTO_STOP_MINUTE_IST;
};
...
const executeSimulationToLiveHandoff = async (userId, session) => {
    ...
    const liveStartResult = await startTrading(userId, {
        session_id,
        saved_configuration_id: session.saved_configuration_id.toString(),
        strategy: "C"
    });
...
const autoStopDueSimulations = async () => {
    if (!isSimulationAutoStopDue()) return;

    const today = getTodayTradeDateIst();
    if (lastAutoStopDateKey === today) return;
    ...
    if (sessions.length === 0) {
        logger.info("Simulation auto-stop: no active sessions for today", { tradeDate: today });
        return;
    }
