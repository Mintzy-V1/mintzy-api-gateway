...
const verifyTotp = async (userId, payload = {}) => {
    ...
    logger.info("Verifying Firstock TOTP", { userId, session_id });
...
const startTrading = async (userId, payload = {}) => {
    ...
    logger.info("Starting Firstock trading session", { userId, session_id });
    ...
    if (isLivePluginStatus(pluginStatus)) {
        ...
        logger.info("Firstock plugin already live; skipping duplicate start", {
            userId,
            session_id,
            pluginStatus
        });
...
const stopSimulationTrading = async (userId, sessionId, options = {}) => {
    ...
    logger.info("Stopping Firstock simulation worker", { userId, sessionId });
...
const stopTrading = async (userId, sessionId) => {
    logger.info("Stopping Firstock session", { userId, sessionId });
...
const stopTradingSymbol = async (userId, sessionId, symbol) => {
    logger.info("Exiting Firstock symbol from session", { userId, sessionId, symbol });
