...
const resolveTargetForSession = async (sessionId) => {
    ...
    const ts = await mongoose.model("TradingSession").findOne({ python_session_id: sessionId }).lean();
...
const getSessionStatus = async (sessionId, userId) => {
    logger.info("Fetching Firstock session status", { sessionId });
...
const getTradingSnapshot = async (sessionId, userId) => {
    logger.info("Fetching Firstock trading snapshot", { sessionId });
...
const getPyramidPnlUncached = async (sessionId, userId) => {
    ...
    logger.info("Fetching Firstock pyramid PnL snapshot", { sessionId, userId });
...
const getAdminTradingLogs = async (limit) => {
    logger.info("Fetching Firstock admin trading logs", { limit });
...
const getAdminTradingLogsByUser = async (userId, limit) => {
    logger.info("Fetching Firstock admin trading logs by user", { userId, limit });
    const params = limit ? { limit: String(limit) } : {};
...
const downloadAdminTradingLogsByUser = async (userId) => {
    logger.info("Downloading Firstock admin trading logs by user", { userId });
...
const getDebugSnapshot = async () => {
    logger.info("Fetching Firstock debug snapshot");
    const pluginRes = await proxyGet("/api/debug/snapshot");
    return pluginRes?.data;
};

const insertFakeTrades = async (sessionId) => {
    logger.info("Inserting Firstock fake trades (debug)", { sessionId });
